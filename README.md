...
and awaiting acceptance; nothing below 4c is built.** What exists is one binary
...
running session's output to a browser tab you can reply from — the one surface
that carries a client layer, and the only place a live terminal makes that
unavoidable.
