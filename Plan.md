...
| 5c | not started. Raised 2026-08-25 out of 5b's review, and inserted before 6 because a second person signing in needs `--accounts` on, which needs this | [MUS-Q-0051](records/questions.md#mus-q-0051), and the measurement behind it: `POST /mcp` on a server with `--accounts` answers 403 to every caller with no session cookie |
