...
| Human interface | **Server-rendered HTML by default, with a client layer only where a surface needs one.** No per-project **server** state per client. **Every page that renders the bar carries a script now, and every page works without it but two.** `bar.js` polls the decision count and writes the badge, on the six surfaces that draw the bar, since [MUS-Q-0078](records/questions.md#mus-q-0078) — until 2026-09-10 this row said intake and the decision queue carry no script at all, which stopped being true on 2026-09-04. Sign-in and accepting an invitation draw no bar and carry no `bar.js`; what they carry is the ceremony script, and the page shown when an invitation has already been used carries nothing at all. Every page still renders its own count server-side; what stops without script is the number changing. Seven surfaces carry a script for something only script can do — the session view, the composer, intake and the four authentication surfaces, and on two of those four it is the only script on the page rather than a second — and two of the seven stop working without it: the session view, which is a live terminal, and the passkey ceremony, which is a browser API. The composer was taken deliberately on [MUS-Q-0034](records/questions.md#mus-q-0034) because a draft cannot survive a backgrounded phone; the account page on [MUS-Q-0047](records/questions.md#mus-q-0047), because adding a passkey had had a page of its own that held a heading and one button; intake's draft on MUS-Q-0120, because the owner leaves the box to look something up and came back to it empty (MUS-F-0152). All three post their forms and work with the script blocked. What the count should measure is answered on [MUS-Q-0053](records/questions.md#mus-q-0053) — script tags, and a seventh is a new decision again, which is how intake's was taken; this row called it open until MUS-F-0153 — and [MUS-Q-0078](records/questions.md#mus-q-0078) moved the numbers rather than settling it. The session view's client layer holds one socket and the screen as it stands: there is no byte offset, no buffer and no replay since [MUS-D-0132](records/decisions.md#mus-d-0132), and this row described an offset until 2026-09-10. `/sessions` redirects into the session view whenever anything is running, so the only unscripted rendering of it is the empty state — and the start page itself when it is asked for by name, which is how a session is started and how a lost one is offered back ([MUS-D-0146](records/decisions.md#mus-d-0146), [MUS-D-0150](records/decisions.md#mus-d-0150)). The exception is the session view at milestone 4b, which streams a live session over a WebSocket and cannot be server-rendered; the owner took that on [MUS-Q-0008](records/questions.md#mus-q-0008) and corrected this row forward rather than reopening milestone 2, on [MUS-Q-0011](records/questions.md#mus-q-0011). The exception is named so the rule is not quietly dropped. |
