...
| Agent transport | A per-machine adapter that starts and supervises long-lived sessions **inside tmux**, shelling out to the configured CLI. Sessions survive adapter restarts and stay attachable from a terminal; Mustur still never attaches to a session it did not start. They do **not** survive a reboot — nothing does — so what is kept across one is a note of what was launched and which conversation it was having, and a button that brings it back ([MUS-D-0149](records/decisions.md#mus-d-0149)) — reading **Resume it** where the conversation comes back off the transcript and **Start it again** where there is none, because one label for both is the defect the owner reported as [MUS-F-0116](records/findings.md#mus-f-0116). Vendor neutrality is a designed boundary **except where a capability belongs to one vendor**: sub-agent visibility and, from milestone 8, the gate that holds a tool call in front of the owner both ride on Claude Code's own lifecycle hooks, so `Start` appends them to a command whose program is that CLI and leaves anything else exactly as given, showing no rows rather than failing to start. The owner named the boundary that way on [MUS-Q-0028](records/questions.md#mus-q-0028) rather than let it be crossed quietly. Only Claude Code is proven. |
| Human interface | **Server-rendered HTML by default, with a client layer only where a surface needs one.** No per-project **server** state per client. **Every page that renders the bar carries a script now, and every page works without it but two.** `bar.js` polls the decision count and writes the badge, on the six surfaces that draw the bar, since [MUS-Q-0078](records/questions.md#mus-q-0078) — until 2026-09-10 this row said intake and the decision queue carry no script at all, which stopped being true on 2026-09-04. Sign-in and accepting an invitation draw no bar and carry no `bar.js`; what they carry is the ceremony script, and the page shown when an invitation has already been used carries nothing at all. Every page still renders its own count server-side; what stops without script is the number changing. Eight surfaces carry a script for something only script can do — the session view, the session list, the composer, intake and the four authentication surfaces, and on two of those four it is the only script on the page rather than a second — and two of the eight stop working without it: the session view, which is a live terminal, and the passkey ceremony, which is a browser API. The composer was taken deliberately on [MUS-Q-0034](records/questions.md#mus-q-0034) because a draft cannot survive a backgrounded phone; the account page on [MUS-Q-0047](records/questions.md#mus-q-0047), because adding a passkey had had a page of its own that held a heading and one button; intake's draft on MUS-Q-0120, because the owner leaves the box to look something up and came back to it empty (MUS-F-0152); the session list on MUS-Q-0161 (MUS-D-0199), because its picker is bound by `session.js`, which it loaded before anybody counted it. All four post their forms and work with the script blocked — the session list's picker through its noscript Go button. What the count should measure is answered on [MUS-Q-0053](records/questions.md#mus-q-0053) — script tags, and a seventh is a new decision again, which is how intake's was taken, and the session list's was the eighth on MUS-Q-0161 (MUS-D-0199); this row called it open until MUS-F-0153 — and [MUS-Q-0078](records/questions.md#mus-q-0078) moved the numbers rather than settling it. The session view's client layer holds one socket and the screen as it stands: there is no byte offset, no buffer and no replay since [MUS-D-0132](records/decisions.md#mus-d-0132), and this row described an offset until 2026-09-10. `/sessions` redirects into the session view whenever anything is running, and it and the start page it serves by name — which is how a session is started and how a lost one is offered back — load `session.js` whenever there is a running or lost session for the picker to list (MUS-D-0199), so the only rendering of it without that script is the one with nothing running and nothing lost ([MUS-D-0146](records/decisions.md#mus-d-0146), [MUS-D-0150](records/decisions.md#mus-d-0150)). The exception is the session view at milestone 4b, which streams a live session over a WebSocket and cannot be server-rendered; the owner took that on [MUS-Q-0008](records/questions.md#mus-q-0008) and corrected this row forward rather than reopening milestone 2, on [MUS-Q-0011](records/questions.md#mus-q-0011). The exception is named so the rule is not quietly dropped. |
...
| Plans held, drawn and reviewed in Mustur, with their questions linked both ways | | yes, milestones 9 and 9b | |
...
| ID expansion as a rendering trick over someone else's prose | It was retrofitting addresses onto text that has none. The numbers this row used to give — 10 of 247 decision identifiers under a heading — were stale by a factor of twenty-seven on the count and fifteen on the proportion: 271 of the 441 distinct decision numbers LinkCtrl's decisions.md mentions have a heading of their own today, against the 10 of 247 this row claimed, because it adopted a per-decision heading at D16 ([MUS-F-0118](records/findings.md#mus-f-0118)). Where the row is right, and by more than it claimed, is findings: all 381 rows in that repository's deferred-findings file are table rows and none is addressable at all. Records Mustur owns are addressable when written. |
...
| 7 | A second project moves in | For LinkCtrl, the first project onboarded with records of its own: every record Mustur has a kind for is in the store and reconciled per source to LinkCtrl's tree; the files holding them are gone from LinkCtrl, with every link into them rewritten and its gates green; and a LinkCtrl session reaches `mustur_route` with a token ([MUS-D-0170](records/decisions.md#mus-d-0170)). It proves the transition the record shape was chosen to test. |
...
| 9 | A plan is held where its decisions are | A session hands Mustur a plan — a document the size of Hoard's group-sharing plan, which today is a file on the VM that only a shell can open (MUS-F-0144) — and it becomes a record addressable by identifier, rendered on the surface where the owner answers decisions, as readable from a phone as from a desk. A question raised against a plan cites it, the plan shows that question, and whether it has been answered reads the same on both; today refs point one way and no surface shows a cited question's state. A plan in another project's checkout stays that project's file: what Mustur holds is what the session handed over. Scheduled on the owner's answer to MUS-Q-0121, plans first (MUS-D-0176). |
| 9b | A plan is drawn in Mustur | The wireframes a surface is planned in are drawn in Mustur rather than in an external tool, quickly enough that drawing one is not the slow step, and freely enough that the surface built looks like the surface planned: the external tool refuses SVG ([MUS-F-0048](records/findings.md#mus-f-0048)) and drew Hoard a claim prompt the app never had. Feedback can be left anywhere on a drawing, which is the one thing the owner named the external tool as doing well, and a question raised from a drawing is 9's question — linked, shown on both, answered on both. Nothing is embedded: a drawing is not a frame around a backend. Split from 9 on the same answer, because a drawing's questions need 9's plan to link to (MUS-F-0148). |
...
| LinkCtrl declares no StrucGu adoption | It is the largest and most active corpus and its `M`/`F`/`D` shape predates the spec — StrucGu was built from the structure LinkCtrl started and the transition never happened. Onboarding it means a mapping exercise, not a lift-and-shift. That repository's agents surveyed their side; the mapping itself is built in Mustur by MUS-M-0009 ([MUS-W-0024](records/work-units/MUS-W-0024.md)). |
...
9. No file in any project other than Mustur has been modified by Mustur, or by
   a session working on Mustur, before that project's onboarding is
   deliberately started. A project's own agents preparing their side in their
   own tree is not that ([MUS-D-0171](records/decisions.md#mus-d-0171)).

---

## Build status

**Milestones 1, 2 and 7 have passed; 7, LinkCtrl moving in, was accepted on 2026-09-15
([MUS-D-0185](records/decisions.md#mus-d-0185)).** **2b, 2c and 3 are built and reviewed; 2c is now
...
| 3 | built and reviewed 2026-08-21, split by the owner so that injection moves to milestone 4. An open question blocks `make check` when it was never surfaced, or when it is marked as one the work depends on; the queue is answerable from a phone | `records/questions.md`, `make questions` over it — over the store instead since 2026-09-14, because nothing Mustur does reads the export ([MUS-D-0183](records/decisions.md#mus-d-0183)) — and the questions this repository has raised — including the four carrying `Blocks: MUS-M-0005`, which stopped this milestone's own build until they were surfaced. The count is in [records/README.md](records/README.md) rather than restated here, because restating it is how it went stale twice |
...
| 6 | not met, and closer than the table above says. The invitation, the ceremony, the role and the reading surfaces exist, a second account holds a passkey and a reader role on MUS, and since 2026-09-10 the tab bar no longer offers that reader the one page that refuses them ([MUS-F-0126](records/findings.md#mus-f-0126)). **What is missing is the reading**: as of 2026-09-10 that person has not looked through Mustur ([MUS-D-0157](records/decisions.md#mus-d-0157)) | The verdict cannot come from the store: nothing in it records a page being read. The server does write a line per request to the journal, naming the account that made it ([MUS-F-0162](records/findings.md#mus-f-0162)), but only from the build that deployed it onward and only for as long as the journal keeps it, so a reading before then left no trace anywhere. It comes from the owner's word or that person's, the way 2c's last clause came from the owner filing a jot from their own phone |
| 7 | passed 2026-09-15: LinkCtrl's 1,503 records in the store and reconciled, its record files gone from LinkCtrl with its CI green, and LinkCtrl sessions reaching `mustur_route` with a token | [MUS-D-0185](records/decisions.md#mus-d-0185), [MUS-W-0024](records/work-units/MUS-W-0024.md), LinkCtrl PR 14 |
| 10 onwards | not started. Milestone 6's reading surface arrived early, inside 5b's branch — a scope decision taken in a commit message rather than asked, and left standing on [MUS-Q-0054](records/questions.md#mus-q-0054) | |
| 9, 9b | not started. Scheduled 2026-09-14 on the owner's answer to MUS-Q-0121 (MUS-D-0176), from MUS-F-0144 and MUS-F-0148. 9's rendering of a plan will use goldmark, taken on MUS-Q-0119 (MUS-D-0177) | |
