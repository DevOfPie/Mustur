...
| Human interface | **Server-rendered HTML by default, with a client layer only where a surface streams.** No per-project **server** state per client, and no script on any surface but one. Intake and the decision queue carry no script, stylesheet, font or image; the session list is not a third such surface and this row used to imply it was — `/sessions` redirects into the session view whenever anything is running, so the only unscripted rendering of it is the empty state; the session view carries a client layer that holds one socket and the byte offset it has reached, which is the per-tab state a stream cannot avoid. The exception is the session view at milestone 4b, which streams a live session over a WebSocket and cannot be server-rendered; the owner took that on [MUS-Q-0008](records/questions.md#mus-q-0008) and corrected this row forward rather than reopening milestone 2, on [MUS-Q-0011](records/questions.md#mus-q-0011). The exception is named so the rule is not quietly dropped. |
...
| 5 | Composition | The owner composes multi-line, spell-checked text from the phone, off the home network, without a terminal, and it reaches the intended session. Built into the session view rather than as a page of its own: a composer needs the client layer, and a second scripted surface is a decision rather than a consequence. **Thought first, destination second** — the plan settled that on 2026-08-20 and this file's brief went on calling it open — so there is one draft rather than one per session, kept as it is typed and surviving a reload, a dropped connection and a backgrounded phone. Multi-line reaches the session as a bracketed paste, because a newline typed into a terminal is Enter and Enter submits ([MUS-D-0096](records/decisions.md#mus-d-0096)). |
...
| 5 | built 2026-08-23, not yet reviewed. The composer is multi-line, spell-checked and drafted: Enter inserts a newline, the Send button submits, the destination is named above the box, and the draft is kept as it is typed under one key so changing session does not cost it | Real tmux and the real CLI. Multi-line left the composer over a real WebSocket and arrived in a real session's input, in order and with its newlines. That a bracketed paste reaches an agent's composer as *one* message rather than one prompt per line was measured by hand — four lines, one Enter, and the agent answered from all four — because no test in this tree can prove it: the pane holds `cat`, and `cat` is happy with newlines either way. Method, and what is still the owner's to confirm, in [records/work-units/MUS-W-0019.md](records/work-units/MUS-W-0019.md) |
