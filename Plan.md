...
| Human interface | **Server-rendered HTML by default, with a client layer only where a surface streams.** No per-project **server** state per client, and no script on any surface but one. Intake and the decision queue carry no script, stylesheet, font or image; the session list is not a third such surface and this row used to imply it was — `/sessions` redirects into the session view whenever anything is running, so the only unscripted rendering of it is the empty state. **Six surfaces carry script** — the session view, the composer and the four authentication surfaces — and only two of them stop working without it. The composer was taken deliberately on [MUS-Q-0034](records/questions.md#mus-q-0034) because a draft cannot survive a backgrounded phone; the account page on [MUS-Q-0047](records/questions.md#mus-q-0047), because adding a passkey had had a page of its own that held a heading and one button. Both of those post their forms and work with the script blocked, which the session view's socket and the passkey ceremony cannot. What the count should measure is open on [MUS-Q-0053](records/questions.md#mus-q-0053); the session view carries a client layer that holds one socket and the byte offset it has reached, which is the per-tab state a stream cannot avoid. The exception is the session view at milestone 4b, which streams a live session over a WebSocket and cannot be server-rendered; the owner took that on [MUS-Q-0008](records/questions.md#mus-q-0008) and corrected this row forward rather than reopening milestone 2, on [MUS-Q-0011](records/questions.md#mus-q-0011). The exception is named so the rule is not quietly dropped. |
