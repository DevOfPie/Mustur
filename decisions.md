...
| [The mandate is the wording the disproof measured](#the-mandate-is-the-wording-the-disproof-measured) | Shipping the clause that was scored, not a descendant |
...
building it, and three of them were the owner's answer to a prompt.
...
Everything this repository already held is in the store and addressable:
twenty-one decisions, nine milestones, three findings, the accepted
investigation. What is *in* each record is a title, a date, and one line — plus
...
[`records/`](records/), regenerated by `mustur export`, committed.
...
Two things were added and both are stated here so the difference from the
scored fixture is on the record:
